...
TThostFtdcOrderStatusType={'0':"全部成交",'1':"部分成交还在队列中","2":"部分成交不在队列中","3":"未成交还在队列中","4":"未成交不在队列中","5":"撤单",
"a":"未知","b":"尚未触发","c":"已触发"}
    ...
    def OnRspQryInvestorPosition(self, pInvestorPosition:"CThostFtdcInvestorPositionField ",  pRspInfo:"CThostFtdcRspInfoField", nRequestID:"int",bIsLast: "bool"):
        ...
        if bIsLast:
            for symbol in self.position:
                ...
                all_PositionCost=self.position[symbol]["PositionCost_short_today"]+ self.position[symbol]["PositionCost_short_his"]
                short_c=self.position[symbol]["pos_short_today"]+self.position[symbol]["pos_short_his"] *self.symbol_v[symbol.split(".")[1]]
            ...
            #更新订单的交易所 和 成交价格
                ...
                self.init_start=1
                print("完成本账户初始化")
            self.queue.put({"user":self.UserID,"position":self.position,"order":self.order,"account":self.account,"trade":self.trade})
        ...
        if  self.init_start is None:
            if pOrder.OrderLocalID not in self.order:
                self.order[pOrder.OrderLocalID]={
                    ...
                    "insert_date_time": time.mktime(time.strptime(pOrder.InsertDate +pOrder.InsertTime,'%Y%m%d%H:%M:%S'))*1e9,
            ...
            if symbol not in self.position:
                ...
                if pTrade.OffsetFlag=='0' :
                    ...
                    if pTrade.Direction =="0":
                        ...
                        'open_price_long':  (self.position[symbol]["open_price_long"]*self.position[symbol]["pos_long"]+pTrade.Price*pTrade.Volume)/(self.position[symbol]["pos_long"]+pTrade.Volume),
                        "position_price_long": (self.position[symbol]["position_price_long"]*self.position[symbol]["pos_long"]+pTrade.Price*pTrade.Volume)/(self.position[symbol]["pos_long"]+pTrade.Volume),
                        ...
                        'open_price_short':  (self.position[symbol]["open_price_short"]*self.position[symbol]["pos_short"]+pTrade.Price*pTrade.Volume)/(self.position[symbol]["pos_short"]+pTrade.Volume),
                        "position_price_short": (self.position[symbol]["position_price_short"]*self.position[symbol]["pos_short"]+pTrade.Price*pTrade.Volume)/(self.position[symbol]["pos_short"]+pTrade.Volume),
                        "pos": self.position[symbol]["pos"]-pTrade.Volume
                        }
                    ...
                    if self.symbol[pTrade.InstrumentID] in ("INE","SHFE"):
                        ...
                        if pTrade.OffsetFlag=='3':
                            ...
                            if pTrade.Direction =="0":
                                d={ "pos_short_today":self.position[symbol]["pos_short_today"]-pTrade.Volume,
                                'pos_short':self.position[symbol]["pos_short"]-pTrade.Volume,
                                "open_price_short": self.position[symbol]["open_price_short"]    if self.position[symbol]["pos_short"]-pTrade.Volume >0  else 0,
                                "position_price_short": self.position[symbol]["position_price_short"]    if self.position[symbol]["pos_short"]-pTrade.Volume >0  else 0,
                                "pos":self.position[symbol]["pos"]+pTrade.Volume
                                }
                                self.position[symbol].update(d)
                            else:
...
#             order_insert(tradespi,"gaoguole","SHFE.cu2010","BUY","OPEN",1,51000,"GFD")
#             time.sleep(10)
#             print("撤单")
#             #order_insert(tradespi,"gaoguole","SHFE.rb2101","BUY","OPEN",1,3500,"GFD")
#             order_close(tradespi,'gaoguole')
